...
通过记忆机制，智能体只需从记忆中检索与当前任务最相关的信息，而非将所有历史数据塞入上下文。这种“按需取用”的模式显著**降低了上下文负担**。例如，Mem0系统通过动态提取、整合对话关键信息，并结合图结构存储实体与关系，在基准测试中不仅性能超越多种基线，还大幅降低了计算开销，p95延迟降低91%，token成本节省90%。
...
具体而言，智能体可以定期总结近期行为，识别模式、错误和改进点。例如，通过设计如下反思提示：
...
本章从三个维度对智能体记忆进行分类：按保存形态分为显式与隐式记忆；按时间跨度分为短期与长期记忆；按用途分为场景、语义与程序记忆。这种多角度的分类体系有助于我们理解记忆系统的复杂结构。
...
**短期记忆（Short Memory）** 涉及当前会话或近期交互的信息，通常通过对话摘要维持，将长交互历史压缩后重新注入上下文窗口。

**长期记忆（Long Memory）** 则跨越多个会话，保留历史经验和关键信息。早期多采用检索增强生成（RAG），将历史交互存储于向量数据库中并按需检索。M+框架在MemoryLLM基础上引入了可扩展的长期记忆，通过把“过期”隐藏向量写入CPU侧长期记忆池，再用协同检索器拉回最相关记忆，将8B级模型的有效记忆跨度从原本不到20k tokens提升到160k tokens以上。
...
反思结果可以作为元记忆存储，用于指导未来任务。A-MEM系统支持**记忆进化**——随着新记忆的整合，它们可以触发现有历史记忆的情境表示和属性的更新，从而使记忆网络能够不断优化其理解。
...
潜在记忆是一种新兴的记忆实现范式，它将记忆信息编码到模型的潜在空间中，通过动态更新的隐状态向量来存储和检索知识。与基于检索的记忆相比，潜在记忆实现了记忆系统与模型推理过程的深度集成；与参数化记忆相比，潜在记忆无需反向传播，更加高效；并且没有**灾难性遗忘**的问题。

#### 5.3.1 核心概念与工作原理

潜在记忆的核心思想是将记忆表示为**可学习的潜在向量**，这些向量在模型的前向传播过程中动态更新和利用。根据《Latent Memory: A New Paradigm for Memory in Large Language Models》的研究，潜在记忆通过以下机制工作：
...
#### 5.3.3 典型案例： MemoryLLM、M+

**MemoryLLM**开创性地实现了参数化记忆，通过在前向传播中持续更新潜在记忆token，使知识得以自动内部更新。具体而言，MemoryLLM在Transformer每一层都加入N个Memory Tokens（实验中N=12800），在生成过程中，这些Memory Tokens会作为每一层的Prefix，通过Cross-Attention将信息注入后续层，使模型能“看到”保存在Memory Pool中的历史信息。

**M+** 则在MemoryLLM之上，进一步提出的长期隐空间记忆扩展框架。核心创新在于将“过期”隐藏向量写入CPU侧长期记忆池，再用协同检索器拉回最相关记忆。这一设计将8B级模型的有效记忆跨度从原本不到20k tokens提升到160k tokens以上，同时显存占用保持不变。
