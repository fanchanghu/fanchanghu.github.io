...
### 1.2 编程语言设计的取舍：运行效率、开发效率与安全的三角

不同的编程语言为了解决不同的问题，形成了自己初始的世界观和价值观。而这些世界观和价值观，会严重影响编程语言设计上的取舍。

传统上，编程语言在**运行效率**和**开发效率**之间做取舍：低级语言（Assembly/C）运行效率高但开发效率低，高级语言（Python/JavaScript）开发效率高但运行效率低。而 Rust 作为系统级语言，还引入了一个被长期忽视的维度——**安全**（内存安全与并发安全）。于是，取舍变成了一个三角：

| 语言 | 运行效率 | 开发效率 | 安全 | 取舍策略 |
|------|---------|---------|------|---------|
| C | ★★★★★ | ★★☆☆☆ | ★☆☆☆☆ | 极致效率，安全全靠程序员 |
| C++ | ★★★★☆ | ★★☆☆☆ | ★★☆☆☆ | 高效 + 抽象，安全依赖自律与最佳实践 |
| Rust | ★★★★☆ | ★★★☆☆ | ★★★★★ | 编译期保证安全，开发效率前期受损 |
| Go | ★★★☆☆ | ★★★★☆ | ★★★★☆ | GC + CSP，用少量运行效率换工程效率 |
| Java | ★★☆☆☆ | ★★★★☆ | ★★★★☆ | GC 保内存安全，并发安全靠规范 |
| Python | ★☆☆☆☆ | ★★★★★ | ★★★☆☆ | 极致开发效率，运行效率垫底 |

**Assembly/C/C++：高运行效率，安全靠程序员**

为了追求极致的运行效率，牺牲了安全和开发效率。内存错误和并发 bug 成为系统软件的顽疾，开发难度大、门槛高。

**Java 等 GC 语言：牺牲运行效率换内存安全和开发效率**
...
以上无论内存安全还是并发安全的解决方案，都有不菲的代价。这对于把运行效率和安全作为核心要素的 Rust 来说是不可接受的。

### 1.3 Rust：运行效率与安全优先，兼顾开发效率

面对三角取舍，大多数语言选择了妥协：C/C++ 牺牲安全换运行效率，Java/Go 牺牲运行效率换开发效率和安全。Rust 的回答是：**运行效率与安全优先，兼顾开发效率**。
...
Rust 对运行效率有一种偏执：语言提供的抽象必须尽可能零成本——你不用，就没有成本；你使用，手写代码通常也不比语言生成的更快。因为这种偏执，Rust 在 1.0 版本时毅然拿掉了 green thread（即便不用，其运行时开销也在那里），直到数年后的 1.39 版本，async/await 才以真正的零成本抽象回归。rust 的实践证明：**高级的抽象并不必然以牺牲运行效率为代价。**

需要诚实地说，Rust 的**开发效率**是它的短板：所有权、借用检查、显式生命周期标注等机制让语法复杂度明显高于其他高级语言，学习曲线陡峭，初学者常常要与编译器"搏斗"。Rust 的策略是：**在保证运行效率和安全这两个底线的基础上，尽可能提高开发效率**——通过模式匹配、闭包、迭代器、泛型、Trait 等现代语言特性，把"能编译通过"的代码写得优雅而高效。

这种运行效率与安全优先，兼顾开发效率的理念，代价是什么呢？答案是：将成本从运行时转移到编码期和编译期。

**代价一：牺牲编码期的便利性（开发效率），换取编译期的强安全。**
...
当然，回报也是巨大的：**无需垃圾回收的内存安全，以及编译期保证的并发安全**。程序员在编码时感到束手束脚，但编译通过后，代码的安全性已经得到了证明。
...
这些零成本抽象的背后，是越来越慢的编译速度、愈发臃肿的编译过程文件。这些选择共同构成了 Rust 的独特定位：**编码时更慢更谨慎，编译时更慢更严格，但运行时更快更安全**。

### 1.4 被低估的世界观：公开透明（Explicitness）

Rust 还有一个重要的、被大家低估的世界观：**公开透明**。使用者可以对他所写的代码做到完全了解和掌控。

很多"高级"编程语言会营造一种易于学习的氛围：你不需要了解一切，不需要熟悉计算机工作原理，不需要掌握操作系统的基本知识，你也可以"高效"编程。**——但这种“just so so”的感觉其实是一种假象。** 如果你做的事情仅仅和 CRUD 相关，掌握一些高层次的 API 的确可以完成工作；但当你面临更复杂的系统设计时，当你想成为一名有追求的开发者时，你会遭遇瓶颈——你还是得老老实实构建需要的知识体系。可是当初的"轻松"已经成为负担，就像练习钢琴一开始在手型上走了捷径，随着曲目难度的增高，这个捷径会反噬你。

实际上，这种假象早已被人才市场无情戳破——Java 工程师的确不需要了解内存运作机制也能编程，但面试时，GC 原理、内存泄漏的成因、JVM 工作机制这类问题还是屡见不鲜。原因无他：不了解这一切，你就无法写出高效、安全且设计良好的代码。
...
这一规则不仅保证了单线程下的内存安全，也天然适用于多线程场景：当编译器确保任意时刻要么只有一个可变引用、要么有多个不可变引用时，数据竞争在编译期就被杜绝了。
...
那么，闭包究竟如何捕获外部变量？——按**闭包体内对变量做了什么**自动推导：

| 闭包内部的操作 | 捕获方式 | 效果 |
|---------------|---------|------|
| 只读取（`println!`、`&self` 方法） | 不可变借用（`Fn`） | 原变量仍可用 |
| 修改值（赋值、`&mut self` 方法） | 可变借用（`FnMut`） | 闭包执行期间独占访问 |
| 消耗值（`drop`、`self` 方法如 `into_iter()`、作为返回值） | 获取所有权（`FnOnce`） | 非 `Copy` 类型被 **Move**（原变量失效）；`Copy` 类型被**复制**（原变量仍可用） |

一言以蔽之：**读它 → 借用；改它 → 可变借用；吃掉它/还给它 → 获取所有权**。上面的 `take_str` 就是"还给它"（作为返回值），所以 `s` 被 Move 了。
