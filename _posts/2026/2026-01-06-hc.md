...
残差连接（Residual Connections）是深度学习的基石，它通过缓解梯度消失问题使得训练极深的网络成为可能。然而，残差连接并非万无一失的解决方案，需要在梯度消失和表示坍塌之间做出不同的权衡。这引出了即残差连接的两个主要变体：前置归一化（Pre-Norm）和后置归一化（Post-Norm）。
- **前置归一化** 在每个残差块之前对输入应用归一化操作，有效地解决了梯度消失的问题。然而，它也可能导致深度表示中出现坍塌问题，即更深层的隐藏特征变得高度相似，从而减少了额外层数的贡献。
- **后置归一化** 在每个残差块的输出之后应用归一化，从而降低了隐藏状态对后续层的影响。这种方法可以缓解表示坍塌的问题，但也重新引入了梯度消失的问题。

梯度消失和表示坍塌就像跷跷板的两端。引起这一现象的关键在于：残差连接预先定义了层内输出和输入之间连接的强度。受此启发，作者提出：神经网络能否自主学习连接的最佳强度以提高性能？而这正是超连接的核心思想：**可学习的深度连接和宽度连接**。
