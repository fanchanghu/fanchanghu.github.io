...
3. **记忆查询**：\\( y_t = V_t \text{softmax}(K_t^\top q_t) \\)，其中 \\(K_t, V_t\\) 分别为缓存中前 t 个 key/value 按列堆叠而成的矩阵。
...
Delta Net 的记忆虽然可以动态读写，但查询方式是**前馈**的：`t` 时刻的输出 \\( y_t \\) 只取决于当前 query，时刻之间没有状态传递，缺少经典 RNN 中 \\( h_t = \sigma(W x_t + R h_{t-1}) \\) 那样的**循环项**。

论文 [Going Beyond Linear Transformers with Recurrent Fast Weight Programmers](https://arxiv.org/abs/2106.06295) 提出的 **Delta RNN** 对此的改进很直接：在记忆中增加一个循环快权重矩阵 \\( R_t \\)（与 \\( W_t \\) 一样由慢网络用 Delta 规则生成，但参数相互独立），并把**上一时刻的输出 \\( y_{t-1} \\) 也作为 query 去查询它**。这样， Delta RNN 的记忆“三要素”为：

1. **记忆存储**：\\( \mathcal{M_t} = (W_t, R_t) \\) ，即前馈记忆与循环记忆两个矩阵。
2. **记忆更新**：与 Delta Net 相同，两个矩阵各自独立地按 Delta 规则更新。
3. **记忆查询**：\\( y_t = W_t q_t + R_t \text{softmax}(y_{t-1}) \\)

与 Delta Net 一样，`q` 和 `k` 需要为正并进行归一化处理；由于 \\( y_{t-1} \\) 充当了循环 query，所以用 `softmax` 将其变换为满足相同约束的形式。

从记忆视角看，\\( R_t \\) 的引入使记忆不仅可以被"当前输入"查询，还可以被"历史输出"查询，从而在 FWP 框架内复现了 RNN 的状态循环：记忆随序列不断演化，而不仅仅是被反复读写。
