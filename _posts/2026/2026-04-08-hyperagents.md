...
database "Agent Archive\n(Agent 存档)" as Codebase #Plum

package "Evolutionary Loop (进化循环)" {

    [SuperAgents\n(被选中的父代超智能体)] as ParentAgent #LightBlue

    package "Agent Generation" {
        [Meta Agent\n(元智能体)] as MetaAgent #LightGreen
        [Task Agent\n(任务智能体)] as TaskAgent #LightSalmon
    }

    database "Test Sets\n(各种测试集)" as TestSets #LightYellow
    artifact "Evaluation Reports\n(评估报告)" as EvalReports #White
    artifact "Code Patch\n(代码补丁)" as CodePatch #White

    [New SuperAgent\n(新生代子实例)] as ChildAgent #LightPink

    ParentAgent ..> MetaAgent : 包含
    ParentAgent ..> TaskAgent : 包含

    MetaAgent -> CodePatch : 5. 分析报告\n重构与修改系统代码

    TaskAgent --> TestSets : 2. 在基准测试上尝试求解
    TestSets -> EvalReports : 3. 自动化评测\n生成得分与日志
    EvalReports --> MetaAgent : 4. 作为反馈输入\n指导下一轮进化

    CodePatch -right-> ChildAgent : 6. 应用补丁\n下一代 Agent
}

Codebase --> ParentAgent : 1. 按特定算法，选出实例
Codebase <-- ChildAgent : 7. 作为新的子代加入
@enduml
```
